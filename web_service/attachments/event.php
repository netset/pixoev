		...
		if(!empty($_REQUEST['event_id'])) 
		{
		    ...
		    if(!empty($row['date_to']))
					...
					if($getGuestDetail = comingGuestDetailArray($_REQUEST['event_id'],$IMG_URL))
				{
                    ...
                    /*===coming===========*/
            /*        if($fc=coming($_REQUEST['event_id']))
                    {
                        while($row=mysql_fetch_assoc($fc))
                        {
							$row2[]=$row;
						}
						//echo "<pre>"; print_r($row2); //die('k1');
						if($row2)
						{
							$o=0;
							$n=0;
							$final=array();
							foreach($row2 as $frnd)
							{ 
                                $frnd1=userProfileDetail($frnd['guest_id']);
                                $final[$n]['user_name']=$frnd1[$o]['uname'];
								if(!empty($frnd1[$o]['profile_pic']))
								{                                         
									$final[$n]['profile_pic']=$IMG_URL.$frnd1[$o]['profile_pic'];
								}	
								$o++;	        
							}											
							$guest[$i]['guest_coming']=$final;
						}						
					}
					else
					{
						$guest[$i]['guest_coming']=[];
					}*/
					/*===notcoming===========*/
            /*        if($fc=not_coming($_REQUEST['event_id']))
                    {
                        while($row=mysql_fetch_assoc($fc))
                        {
							$row2[]=$row;
						...
						if($row2)
						{
							...
							foreach($row2 as $frnd)
							{ 
                                $frnd2=userProfileDetail($frnd['guest_id']);
                                $final11[$w]['user_name']=$frnd2[$x]['uname'];
								if(!empty($frnd1[$x]['profile_pic']))
								{                                         
									$final11[$w]['profile_pic']=$IMG_URL.$frnd2[$x]['profile_pic'];
								}	
								$x++;	        
							}	
							//echo "<pre>";print_r($final11);die;										
							$guest[$i]['guest_notcoming']=$final11;
						}
					}
					else
					{
						$guest[$i]['guest_notcoming']=[];
					}*/
                $i++;                                       
				}
